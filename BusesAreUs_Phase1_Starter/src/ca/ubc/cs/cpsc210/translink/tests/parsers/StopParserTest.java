...
public class StopParserTest {
    ...
    public void testErrorData() throws StopDataMissingException,JSONException,IOException{
        StopParser p = new StopParser("stopsedited.json");
        p.parse();
        assertEquals(8522,StopManager.getInstance().getNumStops());
    }

    @Test(expected = JSONException.class)
    public void testErrorFormat() throws StopDataMissingException,JSONException,IOException{
        StopParser p = new StopParser("stopseditedagain.json");
        p.parse();
    }
}
