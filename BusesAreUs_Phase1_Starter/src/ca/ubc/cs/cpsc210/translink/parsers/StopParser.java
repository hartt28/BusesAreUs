...
public class StopParser {
    ...
    public void parseStops(String jsonResponse)
        ...
        try {
            ...
            throw new JSONException("Wrong stop format");
        }
        for (int i = 0; i < stops.length(); i++) {
            stops.getJSONObject(i);
            try {
                parseStop(stops.getJSONObject(i));
            }
            ...
            public void parseStop(JSONObject stop) throws StopDataMissingException,JSONException{
                if (!(stop.has("Name")
                        && stop.has("StopNo")
                        && stop.has("Latitude")
                        && stop.has("Longitude")
                ...
                storeStops(name, stopNo, lat, lon, routes);
            }



    private void storeStops(String name, int stopNo, double lat, double lon, String routes)
    {
